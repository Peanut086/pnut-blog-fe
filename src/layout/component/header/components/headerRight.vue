...
import {h, defineComponent, ref, onMounted, computed, watch} from 'vue'
...
const loginMenus: Array<IIHeaderMenus> = [
  {
    label: '个人中心',
    key: "mine",
    icon: renderIcon(UserIcon)
...
const currentMenus = ref<Array<IIHeaderMenus>>(menus);
...
const judgeLogin = () => {
  let timer = setInterval(() => {
    token.value = LocalStorageUtil.get("token");
    if (token.value) {
      clearInterval(timer);
      currentMenus.value = loginMenus;
    }
  }, 600)
}
